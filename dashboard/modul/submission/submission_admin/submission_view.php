...
group by id_scope ");
              ...
              <div class="form-group">
                    <label for="Jenis Kelamin" class="control-label col-lg-2">Status Verifikasi</label>
                    <div class="col-lg-5">
                    <select id="verifikasi" name="verifikasi" data-placeholder="Pilih Status Verifikasi ..." class="form-control chzn-select" tabindex="2" required="">
                        <option value="all">All</option>
              ...
              <div class="form-group">
                    <label for="Jenis Kelamin" class="control-label col-lg-2">Status Bayar</label>
                    <div class="col-lg-5">
                    <select id="bayar" name="bayar" data-placeholder="Pilih Status Bayar ..." class="form-control chzn-select" tabindex="2" required="">
                        <option value="all">All</option>
...
.modal-content-abs {
  height: 99%;
    ...
    <div class="modal" id="modal_abs" tabindex="-1" role="dialog" aria-labelledby="myModalLabel"> <div class="modal-dialog modal-lg modal-abs"> <div class="modal-content modal-content-abs"><div class="modal-header"> <button type="button" class="close-modal-abstract btn btn-danger" style="float:right;"><span aria-hidden="true"><i class="fa fa-times fa-6"></i></span></button> <h4 class="modal-title">Abstract Detail</h4> </div> <div class="modal-body" id="isi_abs"> </div> </div><!-- /.modal-content --> </div><!-- /.modal-dialog --> </div>
        ...
        <div class="modal" id="modal_verifikasi" tabindex="-1" role="dialog" aria-labelledby="myModalLabel"> <div class="modal-dialog modal-lg modal-verifikasi"> <div class="modal-content modal-content-abs"><div class="modal-header"> <button type="button" class="close-modal-verifikasi btn btn-danger" style="float:right;"><span aria-hidden="true"><i class="fa fa-times fa-6"></i></span></button> <h4 class="modal-title">Verifikasi Abstract</h4> </div> <div class="modal-body" id="isi_verifikasi"> </div> </div><!-- /.modal-content --> </div><!-- /.modal-dialog --> </div>
...
$(".close-modal-verifikasi").on('click', function() {
        $('#modal_verifikasi').modal('hide');
});

      $("#add_abstract").click(function() {
          $.ajax({
              url : "<?=base_admin();?>modul/submission/submission_admin/add_abstract.php",
              type : "GET",
    ...
    $('#modal_abstract').modal({ keyboard: false,backdrop:'static' });
      ...
      var dtb_abstract = $("#dtb_abstract").DataTable({
         ...
         //disable order dan searching pada tombol aksi use "className":"none" for always responsive hide column
            ...
            'ajax':{
              url :'<?=base_admin();?>modul/submission/submission_admin/submission_data.php',
...
$(".table").on('click','.assign-reviewer',function(event) {
        ...
        id = currentBtn.attr('data-id');
            ...
            $.ajax({
           url : "<?=base_admin();?>modul/submission/submission_admin/reviewer_modal.php",
            type : "post",
            data : {id_data:id},
            success: function(data) {
 ...
 $(".table").on('click','.change-paper',function(event) {
        ...
        id = currentBtn.attr('data-id');
            ...
            $.ajax({
           url : "<?=base_admin();?>modul/submission/submission_admin/status_modal_paper.php",
            type : "post",
            data : {id_data:id},
            success: function(data) {
                $("#isi_status").html(data);
    ...
    $(".table").on('click','.change-abstract',function(event) {
        ...
        id = currentBtn.attr('data-id');
            ...
            $.ajax({
           url : "<?=base_admin();?>modul/submission/submission_admin/status_modal.php",
            type : "post",
            data : {id_data:id},
            success: function(data) {
                $("#isi_status").html(data);
    ...
    $(".table").on('click','.verifikasi',function(event) {
        $("#loadnya").show();
        event.preventDefault();
        var currentBtn = $(this);
        ...
        $.ajax({
            url : "<?=base_admin();?>modul/submission/abstract/verifikasi_modal_view.php",
            type : "post",
            data : {id_abstract:id},
            success: function(data) {
                $("#isi_verifikasi").html(data);
                $("#loadnya").hide();
                //dtb_abstract.draw(false);
          }
        });

    $('#modal_verifikasi').modal({ keyboard: false,backdrop:'static' });
